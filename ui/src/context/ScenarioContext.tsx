...
const confirmation  = { name: "Confirmation", path: "confirmation", elem: <App key={3}  label="Confirmation"  description="Confirm OTC derivative trades"           image={appDistribution} path="/app/confirmation/confirmations" /> };
const custody       = { name: "Custody",      path: "custody",      elem: <App key={4}  label="Custody"       description="View positions across your accounts"     image={appCustody}      path="/app/custody/assets" /> };
const defi          = { name: "DeFi",         path: "defi",         elem: <App key={5}  label="DeFi"          description="Explore Decentralized Finance protocols" image={appDefi}         path="/app/defi/exchanges" /> };
const distribution  = { name: "Distribution", path: "distribution", elem: <App key={6}  label="Distribution"  description="Distribute assets in the primary market" image={appDistribution} path="/app/distribution/auctions" /> };
const lending       = { name: "Lending",      path: "lending",      elem: <App key={7}  label="Lending"       description="Borrow and lend securities"              image={appLending}      path="/app/lending/trades" /> };
const servicing     = { name: "Servicing",    path: "servicing",    elem: <App key={8}  label="Servicing"     description="Service and lifecycle your assets"       image={appServicing}    path="/app/servicing/lifecycle" /> };
const simulation    = { name: "Simulation",   path: "simulation",   elem: <App key={9}  label="Simulation"    description="Run market scenarios on your assets"     image={appSimulation}   path="/app/simulation/scenario" /> };
const listing       = { name: "Listing",      path: "listing",      elem: <App key={10} label="Listing"       description="List your assets on trading venues"      image={appListing}      path="/app/listing/listings" /> };
const quoting       = { name: "Quoting",      path: "quoting",      elem: <App key={11} label="Quoting"       description="Request trade quotes from dealers"       image={appTrading}      path="/app/quoting/quotes" /> };
const trading       = { name: "Trading",      path: "trading",      elem: <App key={12} label="Trading"       description="Trade assets in the secondary market"    image={appTrading}      path="/app/trading/markets" /> };
const network       = { name: "Network",      path: "network",      elem: <App key={13} label="Network"       description="Explore the distributed ledger network"  image={appNetwork}      path="/app/network/overview" /> };
const settlement    = { name: "Settlement",   path: "settlement",   elem: <App key={14} label="Settlement"    description="Settle instructions in batches"          image={appSimulation}   path="/app/settlement/batches" /> };
...
export const scenarios : Scenario[] = [
  ...
  {
    label: "Equity Total Return Swaps",
    description: "Trading, lifecycling, and corporate actions for Equity TRSs",
    apps: [ structuring, confirmation, custody, servicing, settlement ],
    image: scenarioSecuritiesLending,
    positions: new Map([
      [ "Operator",         { x:    0, y:   0 } ],
      [ "Public",           { x:    0, y:   0 } ],
