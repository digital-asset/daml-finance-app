...
import { Service, Request, Offer } from "@daml.js/daml-finance-app/lib/Daml/Finance/App/Distribution/Auction/Service"
import useStyles from "../styles";
import { getName, getParty, getTemplateId } from "../../util";
import { InputDialog, InputDialogProps } from "../../components/InputDialog/InputDialog";
import { Role } from "@daml.js/daml-finance-app/lib/Daml/Finance/App/Role/Distributor";
...
export const Distribution : React.FC = () => {
  ...
  const defaultRequestDialogProps : InputDialogProps<any> = {
    ...
    fields: { provider: { label: "Provider", type: "selection", items: parties } },
  ...
  const defaultOfferDialogProps : InputDialogProps<any> = {
    ...
    fields: { customer: { label: "Customer", type: "selection", items: parties } },
    onClose: async function(state : any | null) {}
  };
  ...
  const offerService = () => {
    const onClose = async (state : any | null) => {
      ...
      await ledger.exercise(Role.OfferAuctionService, roles[0].contractId, { customer: getParty(state.customer) });
    };
  ...
  const approveRequest = async (c : CreateEvent<Request>) => {
    ...
    await ledger.exercise(Role.ApproveAuctionServiceRequest, roles[0].contractId, { auctionServiceRequestCid: c.contractId });
  }
              ...
              <Grid container direction="row" justifyContent="center">
                <Grid item xs={6}>
                  <Grid container justifyContent="center">
                    <Button color="primary" size="large" className={classes.actionButton} variant="outlined" onClick={requestService}>Request Listing Service</Button>
                  </Grid>
                </Grid>
                <Grid item xs={6}>
                  <Grid container justifyContent="center">
                    {hasRole && <Button color="primary" size="large" className={classes.actionButton} variant="outlined" onClick={offerService}>Offer Listing Service</Button>}
                  </Grid>
                </Grid>
              </Grid>
            </Paper>
                ...
                <TableBody>
                  {services.map((c, i) => (
                    <TableRow key={i} className={classes.tableRow}>
                      ...
                      <TableCell key={6} className={classes.tableCell}>
                        <IconButton color="primary" size="small" component="span" onClick={() => navigate("/network/listing/service/" + c.contractId)}>
                ...
                <TableBody>
                  {requests.map((c, i) => (
                    <TableRow key={i} className={classes.tableRow}>
                      ...
                      <TableCell key={5} className={classes.tableCell}>
                        <IconButton color="primary" size="small" component="span" onClick={() => navigate("/network/listing/request/" + c.contractId)}>
                          <KeyboardArrowRight fontSize="small"/>
                        </IconButton>
                      </TableCell>
                    </TableRow>
                ...
                <TableBody>
                  {offers.map((c, i) => (
                    <TableRow key={i} className={classes.tableRow}>
                      ...
                      <TableCell key={5} className={classes.tableCell}>
                        <IconButton color="primary" size="small" component="span" onClick={() => navigate("/network/listing/offer/" + c.contractId)}>
                          <KeyboardArrowRight fontSize="small"/>
                        </IconButton>
                      </TableCell>
                    </TableRow>
