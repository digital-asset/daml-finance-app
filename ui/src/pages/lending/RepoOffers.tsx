...
import { RepoOffer } from "@daml.js/daml-finance-app/lib/Daml/Finance/App/Lending/Model";
...
export const RepoOffers : React.FC = () => {
  ...
  const { loading: l3, contracts: offers } = useStreamQueries(RepoOffer);
  ...
  const acceptRepoOffer = async (offer : CreateEvent<RepoOffer>) => {
    ...
    const arg = {
      repoOfferCid: offer.contractId,
      collateralCid: collateralCid as string as ContractId<Transferable>,
      account: borrowerAccount
    };
    await ledger.exercise(Lending.AcceptRepoOffer, lending[0].contractId, arg);
    navigate("/app/lending/trades");
  };

  const createRow = (c : CreateEvent<RepoOffer>) : any[] => {
    return [
      ...
      isCustomer ? <Button color="primary" size="small" className={classes.choiceButton} variant="contained" onClick={() => acceptRepoOffer(c)}>Accept</Button> : <></>
    ];
  }
  const headers = ["Borrower", "Lender", "Id", "Borrowed", "Maturity", "Interest", "Collateral", "Action"]
  const values : any[] = offers.map(createRow);
  return (
    <HorizontalTable title="Repo Offers" variant={"h3"} headers={headers} values={values} />
  );
};
