...
export const NewFloatingRateBond : React.FC = () => {
  ...
  const canRequest = !!issueDate && !!maturityDate && !!rate && !!spread && !!couponFrequency && !!currency && !!label;
  ...
  const rateId = ids.find(id => id.label === rate);

  useEffect(() => {
    if (!issueDate || !maturityDate || !rateId || !spread || !ccyId) return;
    const createBond = async () => {
      ...
      const [c, ] = await ledger.createAndExercise(Factory.CreateFloatingRate, { party }, arg);
      setNode(claimToNode(c));
    };
    createBond();
  }, [ledger, party, issueDate, maturityDate, rateId, spread, couponFrequency, ccyId]);
              ...
              <Grid item xs={12}>
                <Paper className={classnames(classes.fullWidth, classes.paper)}>
                  ...
                  <FormControl className={classes.inputField} fullWidth>
                    <InputLabel className={classes.selectLabel}>Rate</InputLabel>
